    ...
    namespace multiprecision {
        namespace backends {
            ...
            inline BOOST_MP_CXX14_CONSTEXPR
                typename std::enable_if<boost::multiprecision::detail::is_integral<R>::value &&
                ...
                BOOST_IF_CONSTEXPR(numeric_limits_workaround<R>::digits < cpp_int_modular_backend<Bits>::limb_bits) {
                    if (boost::multiprecision::detail::is_signed<R>::value &&
                        boost::multiprecision::detail::is_integral<R>::value &&
                        static_cast<boost::multiprecision::limb_type>((std::numeric_limits<R>::max)()) <=
                            backend.limbs()[0]) {
                ...
                BOOST_IF_CONSTEXPR(numeric_limits_workaround<R>::digits > cpp_int_modular_backend<Bits>::limb_bits) {
                    while ((i < backend.size()) &&
                           (shift < static_cast<unsigned>(numeric_limits_workaround<R>::digits -
                    ...
                    if (i < backend.size()) {
                        const limb_type mask =
                            numeric_limits_workaround<R>::digits - shift == cpp_int_modular_backend<Bits>::limb_bits ?
...
#pragma GCC diagnostic pop
            ...
            template<unsigned Bits>
            inline BOOST_MP_CXX14_CONSTEXPR
                typename std::enable_if<!is_trivial_cpp_int_modular<cpp_int_modular_backend<Bits>>::value>::type
            ...
            inline BOOST_MP_CXX14_CONSTEXPR
                typename std::enable_if<!is_trivial_cpp_int_modular<cpp_int_modular_backend<Bits>>::value>::type
                eval_bit_unset(cpp_int_modular_backend<Bits> &val, std::size_t index) noexcept {
                unsigned offset = index / cpp_int_modular_backend<Bits>::limb_bits;
                unsigned shift = index % cpp_int_modular_backend<Bits>::limb_bits;
                limb_type mask = limb_type(1u) << shift;
                if (offset >= val.size())
            ...
            inline BOOST_MP_CXX14_CONSTEXPR typename std::enable_if<
                is_trivial_cpp_int_modular<cpp_int_modular_backend<Bits>>::value &&
                std::is_convertible<typename cpp_int_modular_backend<Bits>::local_limb_type, R>::value>::type
                eval_convert_to(R *result, const cpp_int_modular_backend<Bits> &val) {
                using common_type =
                    typename std::common_type<R, typename cpp_int_modular_backend<Bits>::local_limb_type>::type;
            ...
            template<unsigned Bits>
            inline BOOST_MP_CXX14_CONSTEXPR std::size_t hash_value(const cpp_int_modular_backend<Bits> &val) noexcept {
                std::size_t result = 0;
...
#pragma warning(pop)
        ...
        namespace detail {
            ...
            // All we have changed here is Backend::signed_types -> Backend::unsigned_types, this will work for our use
            // cases.
            ...
            struct canonical_imp<Val, backends::cpp_int_modular_backend<Bits>, std::integral_constant<int, 0>> {
                static BOOST_MP_CXX14_CONSTEXPR int index =
                    find_index_of_large_enough_type<typename backends::cpp_int_modular_backend<Bits>::unsigned_types,
                                                    0,
                                                    bits_of<Val>::value>::value;
                using type =
                    typename dereference_tuple<index,
                                               typename backends::cpp_int_modular_backend<Bits>::unsigned_types,
                                               Val>::type;
            };
